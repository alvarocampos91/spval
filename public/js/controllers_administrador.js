...
angular.module('alumnoApp.controllers',['angularUtils.directives.dirPagination']).controller('AlumnoListController',function($scope,$state,popupService,$window,Alumno){
    ...
    $scope.currentPage = 1;
    $scope.pageSize    = 7;
    $scope.alumnos     = Alumno.query(function(data){console.log(data);});
    $scope.q           = {};
    ...
    Alumno.get({id:$stateParams.matricula},function(alumno)
    {
        $scope.alumno = alumno;
        Profesor.get({dni:$scope.alumno.tutor},function(data)
        {
            $scope.alumno.tutor=data;
            $scope.alumno.nombreTutor=data.nombre+' '+data.a_paterno+' '+data.a_materno;
        });
        Carrera.get({_id:$scope.alumno.carrera},function(data)
        {
            $scope.alumno.carrera=data;
            $scope.alumno.nombreCarrera=data.nombre;
        });
    });
    ...
    $scope.updateAlumno=function(){
        $scope.alumno.nombreCompleto = undefined;
    ...
    $scope.loadAlumno=function(){
        $scope.alumno=Alumno.get({id:$stateParams.matricula});
        $scope.carreras=Carrera.query();
        $scope.profesores=Profesor.query();
    };
